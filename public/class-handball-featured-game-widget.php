...
class HandballFeaturedGameWidget extends WP_Widget
{
    ...
    public function widget($args, $instance)
    {
        ...
        if ($team == null) {
          ...
          $match = $this->matchRepo->findNextMatch($team);
          if ($match == null) {
            echo "Kein Spiel angesetzt";
          } else {
            echo $this->renderEvent($match);
          }
        }
        echo '</div>';
    ...
    private function renderEvent(Game $game): string {
        ...
        return "
          <a style='display:".$hidden.";color:white;font-weight:bold;width:40px;text-align:center;height:170px;;padding-top:70px;float:right;background-color: var(--orange)' href='".$postUrl."'>></a>
          <div class='clearfix' style='padding-top:15px;'>
            <img src='".$imgUrl."' /><br />".esc_attr($game->getOpponentTeamName())."
            <br />
            <span style='font-size:14px;'>".esc_attr($game->getGameDateTimeFormattedLong())." Uhr in ".$game->getVenue()."</span> 
          </div>
        ";
    }
